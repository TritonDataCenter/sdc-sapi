...
function sanitizeMetadata(metadata) {
	...
	var allowed_keys = [ 'SAPI_URL', 'sapi_url', 'SAPI-URL', 'sapi-url',
	    'user-script', 'assets-ip', 'SAPI_MODE' ];
...
function generateZoneParams(zoneuuid, cb) {
	var self = this;
	var log = self.log;
	...
	async.waterfall([
		function (subcb) {
			getParentObjects.call(self, zoneuuid, subcb);
		},
		function (objs, subcb) {
...
function generateZoneConfig(zoneuuid, cb) {
	var self = this;
...
function validOwnerUuid(owner_uuid, cb) {
	...
	log.debug('validating owner_uuid %s', owner_uuid);
	...
	ufds.getUser(owner_uuid, function (err, user) {
		...
		log.debug({ user: user }, 'found owner_uuid %s', owner_uuid);
