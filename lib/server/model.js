...
function deployZone(params, wait, cb) {
	var self = this;
	var log = self.log;

	assert.object(params, 'params');
	assert.bool(wait, 'wait');
	...
	self.vmapi.createVm(params, function (err, res) {
		...
		/*
		 * If the wait flag is true, wait for the VMAPI job to complete
		 * before returning.  Otherwise, just return immediately.
		 */
		if (wait)
			self.vmapiplus.waitForJob(res.job_uuid, cb);
		else
			cb(null);
...
Model.prototype.deployInstance = function (inst, wait, cb) {
	var self = this;
	var log = self.log;
	...
	async.waterfall([
		...
		function (params, subcb) {
			deployZone.call(self, params, wait, subcb);
		}
	...
	async.waterfall([
		...
		function (subcb) {
			...
			resolveManifests.call(self, attributes.manifests,
				 ...
				 * config's metadata.  It clutters up the log,
