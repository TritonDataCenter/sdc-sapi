...
function generateZoneParams(uuid, cb) {
	...
	async.waterfall([
		...
		function (objs, subcb) {
			/*
			 * There is no zone for a non-vm service type, so we return an
			 * empty params object
			 */
			if (objs.service.type !== 'vm') {
				log.info('no generateZoneParams for %s, service type is %s',
...
function generateZoneConfig(uuid, cb) {
	...
	async.waterfall([
		...
		function (objs, subcb) {
			...
			attributes.metadata.ZONE_UUID = objs.instance.uuid; // deprecated
