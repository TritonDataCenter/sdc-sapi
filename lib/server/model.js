	...
	async.waterfall([
		...
		function (record, subcb) {
			...
			self.stor.putObject(bucket, uuid, obj, opts,
				function (err) {
				if (err && err.name === 'EtagConflictError' && tries > 0) {
...
Model.prototype.createService = function createService(svc, cb) {
	...
	async.waterfall([
		...
		function (subcb) {
			...
			self.getApplication(svc.application_uuid, function (err) {
				subcb(err);
			});
		},
		function (subcb) {
...
Model.prototype.createInstance = function createInstance(inst, cb) {
	...
	async.waterfall([
		...
		function (subcb) {
			self.getService(inst.service_uuid, function (err, instService) {
				service = instService;
				subcb(err);
			});
		},
		...
		function (subcb) {
			if (service.type !== 'vm') {
				log.info('skipping provision of %s since service type is %s',
					inst.uuid, service.type);
				return (subcb(null));
			}
...
Model.prototype.upgradeInstance = function (uuid, image_uuid, cb) {
	...
	async.waterfall([
		...
		function (subcb) {
			if (service.type !== 'vm') {
				log.info('skipping teardown of %s since service type is %s',
					uuid, service.type);
				return (subcb(null));
			}
		...
		function (subcb) {
			if (service.type !== 'vm') {
				log.info('skipping reprovision of %s since service type is %s',
					uuid, service.type);
				return (subcb(null));
			}
...
Model.prototype.delInstance = function (uuid, cb) {
	...
	async.waterfall([
		...
		function (subcb) {
			if (service.type !== 'vm') {
				log.info('skipping deletion of %s since service type is %s',
					uuid, service.type);
				return (subcb(null));
			}
