...
Model.prototype.createApplication = function createApplication(
	...
	log.info({ app: app, options: options }, 'creating application');
...
Model.prototype.getInstancePayload = function (uuid, cb) {
	...
	this.attributes.generateZoneParamsAndConfig(uuid,
	...
	async.waterfall([
		...
		function (subcb) {
			...
			self.vmapiplus.reprovisionVm(uuid, image_uuid,
			    function (err) {
				if (err) {
					log.error(err, 'failed to upgrade VM');
					return (subcb(err));
				}
...
function deployInstance(inst, cb) {
	...
	async.waterfall([
		function (subcb) {
			self.attributes.generateZoneParamsAndConfig(
