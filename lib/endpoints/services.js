 ...
 *
 * services.js: SAPI endpoints to manage services
 */
...
Services.create = function (req, res, next) {
	...
	model.createService(params, function (err, app) {
		if (err) {
...
Services.get = function (req, res, next) {
	...
	model.getService(uuid, function (err, app) {
		if (err)
			res.send(500);
		else if (!app)
			res.send(404);
...
Services.del = function (req, res, next) {
	...
	model.delService(uuid, function (err) {
		if (err) {
