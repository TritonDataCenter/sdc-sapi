...
function MorayLocalStorage(opts) {
	...
	assert.object(opts.config.log, 'opts.config.log');
	assert.object(opts.config.buckets, 'opts.config.buckets');
	assert.object(opts.moray, 'opts.moray');
	assert.object(opts.local, 'opts.local');

	self.log = opts.config.log;
	self.moray = opts.moray;
	self.local = opts.local;
	self.buckets = opts.config.buckets;
	...
	vasync.pipeline({
		'funcs': [
			...
			function delLocal(_, subcb) {
				self.local.delObject(bucket, uuid, subcb);
			}
		]
	}, cb);
};
