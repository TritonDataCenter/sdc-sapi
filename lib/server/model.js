...
Model.prototype.delInstance = function (uuid, cb) {
	var self = this;
	self.moray.delObject(INSTANCES, uuid, {}, function (err) {
		if (!err)
...
function rewriteInstanceMetadata(app, svc, inst, cb) {
	...
	generateZoneParamsAndConfig.call(self, inst.uuid, opts,
	    function (err, params, _) {
		if (err)
			return (cb(err));

		params.customer_metadata.uuid = inst.uuid;

		log.debug({ customer_metdata: params.customer_metadata },
		    'updating zone\'s metadata');

		self.vmapi.setMetadata('customer_metadata',
		    params.customer_metadata, function (suberr) {
			if (suberr)
...
Model.prototype.createManifest = function (mfest, cb) {
	...
	assert.string(mfest.type, 'mfest.type');
