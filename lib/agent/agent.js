...
Agent.prototype.init = function init(cb) {
	...
	log.info({ localMetadata: this.localMetadata },
...
Agent.prototype.checkAndRefresh = function checkAndRefresh(cb) {
	...
	vasync.forEachParallel({
		func: function (instance, subsubcb) {
			self.checkAndRefreshInstance.call(self, instance, subsubcb);
		},
...
function checkAndRefreshInstance(instance, cb) {
	...
	async.waterfall([
		function querySapi(subcb) {
			sapi.getConfig(instance, function (err, config) {
				if (err) {
				...
				var manifests = resolveManifests.call(self,
				    config.manifests, self.localManifests);
		...
		if (err)
			log.error(err, 'failed to write files for instance %s', instance);
