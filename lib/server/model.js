...
/*
 * Given an application, service, and instance, assemble an object which
 * consists of the specified field from each object.
 *
...
Model.prototype.deployInstance = function (inst, cb) {
	...
	async.waterfall([
		...
		function (subcb) {
			var params = assemble(app, svc, inst, 'params');
			var metadata = assemble(app, svc, inst, 'metadata');
			var configs = concatConfigs(app, svc, inst);
...
Model.prototype.downloadImage = function downloadImage(uuid, cb) {
	...
	async.waterfall([
		...
		function (subcb) {
			mod_images.download.call(self, uuid, subcb);
		},
		function (files, subcb) {
			assert.object(files, 'files');

			mod_images.importImage.call(self, files.manifest,
			    function (err) {
				return (subcb(err, files));
			});
		},
		function (files, subcb) {
