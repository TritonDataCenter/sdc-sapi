...
Model.prototype.createApplication = function (app, cb) {
	...
	assert.optionalObject(app.configs, 'app.configs');
...
Model.prototype.createService = function (svc, cb) {
	...
	assert.optionalObject(svc.configs, 'svc.configs');
...
Model.prototype.createInstance = function (inst, cb) {
	...
	assert.optionalObject(inst.configs, 'inst.configs');
...
Model.prototype.deployInstance = function (inst, cb) {
	...
	async.waterfall([
		...
		function (subcb) {
			...
			assert.optionalObject(attributes.configs);
			...
			mod_config.resolveAll.call(self, configs,
				...
				params.customer_metadata =
				    mod_manifests.sanitize.call(
				    self, params.customer_metadata);
